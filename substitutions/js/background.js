...
chrome.browserAction.onClicked.addListener(function () {
    status = chrome.storage.sync.get("status", function (result){
        ...
        if(status === "enabled"){
            icon  =  {"path": "images/disabled.png"};        
            message = {"title": "click to enabled xkcd substitutions"};
            status = "disabled";
        }else if (status === "disabled"){
            icon = {"path": "images/enabled.png"};    
            message = {"title": "click to disabled xkcd substitutions"};
            status = "enabled";
        }
